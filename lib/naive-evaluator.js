...
function filterBySuit(cards, suit) {
	var filtered = [];
...
var checkStraights = measure(function(hand) {
	...
	for (var i = 0; i < hand.cards.length; ++i) {
		...
		if (card.rank === Card.Rank.ACE) {
			var fakeAce = new Card(Card.Rank.LOW_ACE, card.suit);

			local.add(fakeAce);
		}
	}

	for (var i = 0; i < local.cards.length - 5 + 1; ++i) {
		var last = -1;
		var highCard = 0;
		var isStraight = true;
		var isFlushed = false;
		...
		for (var j = i; j < local.cards.length && count >= 0 ; ++j) {
			var card = local.cards[j];
		...
		for (var flushSuit = 0; flushSuit < 4; ++flushSuit) {
			if ((suitComposite % Hand.FlushComposites[flushSuit]) === 0) {
				flushedSuit = flushSuit;
				isFlushed = true;
				break;
			}
		}
