...
CardSet.prototype.sort = function(comparator) {
	if (typeof(comparator) !== 'undefined') {
		comparator = CardSet.compareByRank;
	}
...
CardSet.compareByRankDesc = function(a, b) {
	return 4 * (a.rank - b.rank) + (a.suit - b.suit);
};

CardSet.compareBySuitDesc = function(a, b) {
	return 15 * (a.suit - b.suit) + (a.rank - b.rank);
};
 ...
 * @api public
 */
CardSet.prototype.add = function(cardset) {
	if (''+cardset === cardset) {
		return this.add(CardSet.parse(cardset));
	}

	var cards = [];

	if (util.isArray(cardset)) {
		cards = cardset;
	} else if (cardset instanceof CardSet) {
		cards = cardset.cards;
	} else if (cardset instanceof Card) {
		cards = [cardset];
	}
...
CardSet.prototype.clear = function() {
	this.cards = [];
 ...
 * @api public
 */
CardSet.prototype.remove = function(cardset) {
	if (''+cardset === cardset) {
		return this.remove(CardSet.parse(cardset));
	}
 ...
 */
CardSet.prototype.peek = function() {
	return this.cards[0];
};

/**
 * Returns the top cards on the deck and removes them
 */
CardSet.prototype.take = function(count) {
...
CardSet.prototype.join = function(cardset) {
	return this.copy().add(cardset);
};
