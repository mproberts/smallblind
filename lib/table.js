...
Table.prototype.finishRound = function() {
	...
	// rank the hands of the remaining players
	...
	for (var i in this.playersInHand) {
		if (ranks[i] === winningRank) {
			winners.push(this.playersInHand[i]);
		}
	}

	// split the pots
	for (var i in winners) {
		winners[i].stack += this.pot.chips / winners.length;
	}

	this.emit('finished', playerIds(winners));
};
